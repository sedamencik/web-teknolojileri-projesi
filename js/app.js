...
async function getmovie(value,pagenr) {
    ...
    result.Search.forEach((item) => {
        ...
        description.innerHTML=`Title : ${item.Title}<br><br>Yılı : ${item.Year}<br><br> <a href=https://www.imdb.com/title/${item.imdbID}> IMDB : https://www.imdb.com/title/${item.imdbID}</a>`;
